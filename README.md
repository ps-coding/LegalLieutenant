...
- **Storing data:** In the initial stages, we stored the user data like username/password within the project files. This is inefficient, poor practice, and above all a safety concern. Over the next few rounds of development, we intended to change to an external database to store our data in order to make it more secure. We researched a variety of different options like PostgreSQL, MySQL, and even the usage of the phpMyAdmin administration tool. In the end, we decided to use Firebase as it is free to get started with, provides in-depth usage statistics to developers, and offers great security and authentication tools. 
...
- **Account system:** We require the user to be logged into a legal lieutenant account in order to access our features. They can make an account or log into one through our registration page. We also have the option to log out, which allows for someone to have multiple accounts or to browse as a guest. 
...
stored in a database rather than being stored in the project files. In order to do this, we store all of our user data through Firebase. In order to enforce the sign-in system to be required when accessing our features, we had to manage routes to make sure that the user could only access our AI features if they are logged in. This is done to prevent automated bot spam. 
...
- [ ] **Google Account compatability:** Currently, the only way to sign into our website is through a legal lieutenant account, which they can create in our registration page. In order to make the sign-in process more simple and accessible for people to use, we intend on adding compatability for users to sign-in with a google account, where we would automatically create a legal lieutenant account using that email address.  
